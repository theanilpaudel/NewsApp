...
import com.anilpaudel.newsapp.viewmodel.NewsViewModel
...
fun NewsScreen(navController: NavController, vm: NewsViewModel = hiltViewModel()) {
    ...
    val newsFlow = vm.newsListData.collectAsState()
    ...
    LaunchedEffect(sourceFlow.value) {
        when (sourceFlow.value) {
            ...
            is DataState.Success -> {
                val sources =
                    (sourceFlow.value as DataState.Success<SourceResponse>).result.sources?.mapNotNull {
                        it.id
                    }?.joinToString(",")
                vm.getNews(sources)
            }
